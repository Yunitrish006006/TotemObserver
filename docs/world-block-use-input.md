...
The debug first-person controls advertise `右鍵使用` only when the server negotiated
block-use capability. A right mouse-button down begins use; E no longer sends use.
Actual pointer capture and primary keyboard focus are required. Only a mouse-button down queues use; holding the button does not repeat interaction.
The browser adapter suppresses contextmenu only while it owns pointer lock. It
removes mouse/contextmenu listeners and clears the callback on disposal.
...
aim is the visible authoritative camera plus unsent deltas at mouse-down, even if
...
replays it after a later response. Input can be retried with a new click.

Tests cover no input before capture, idle look flush before use, one request
while holding the mouse button, suppression of new section work, capture-loss cancellation
...
the real browser/server fixture verifies right-click lever interaction and
partial-block visibility. No new raster assets are introduced.

This slice does not add inventory key handling, partial-block
