...
void main() {
  ...
  testWidgets(
    'right click use flushes idle look before a single authoritative intent',
    (tester) async {
      final (connection, socket) = await connect();
      late TestCapture capture;
      await tester.pumpWidget(
      ...
      expect(
        connection.blockUsePreparing,
        isFalse,
        reason: 'E no longer uses blocks',
      );
      await tester.sendKeyDownEvent(LogicalKeyboardKey.keyW);
      capture.onUse?.call();
      capture.onUse?.call(); // Repeated click cannot queue a second use.
