...
/** Real read-only outline wire capture and hostile binding validation. */
public final class ObserverTargetOutlineProtocolGameTest {
    @GameTest(maxTicks = 100_000)
    public void expiredOrRevokedOutlineCaptureFailsClosed(GameTestHelper helper) {
        ...
        var auth = new ObserverAccountService.Session("outline_" + UUID.randomUUID().toString().substring(0, 8), UUID.randomUUID());
        var result = admissions.open(sessions.open(auth)).thenCompose(admission -> {
            ...
            return admissions.targetOutline(admission, dimension, clock.get() - TimeUnit.MILLISECONDS.toNanos(251), () -> true)
                    .thenCompose(expired -> {
                        require(expired == null, "Expired outline captured");
                        return admissions.targetOutline(admission, dimension, clock.get(), () -> false);
                    }).thenAccept(revoked -> require(revoked == null, "Revoked outline captured"));
        });
        result.whenComplete((ignored, failure) -> { admissions.close(); sessions.close(); });
        helper.startSequence().thenWaitUntil(() -> {
            if (!result.isDone()) helper.fail("Waiting for outline expiry capture");
            result.join();
        }).thenSucceed();
    }
    ...
    public void authenticatedOutlineBindsCameraAndRejectsHostileRequests(GameTestHelper helper) {
        ...
        var result = CompletableFuture.runAsync(() -> {
            ...
            try {
                directory = Files.createTempDirectory("observer-outline-wire-");
                ...
                try (var bridge = new ObserverBridgeServer(accounts, sessions, admissions);
                    ...
                    for (int scenario = 0; scenario < 10; scenario++) {
                        ...
                        try {
                            ...
                            require(hello.get("worldTargetOutlineProtocol").getAsInt() == 1 && !hello.get("play").getAsBoolean(),
                                    "Outline capability/play boundary");
                            String name = "use_" + UUID.randomUUID().toString().replace("-", "").substring(0, 10);
                            send(socket, "{\"type\":\"register\",\"username\":\"" + name
                                    + "\",\"password\":\"outline-fixture-password\"}");
                            ...
                            if (scenario == 1) onServer(server, () -> playerRef.get().setXRot(-90));
                            send(socket, "{\"type\":\"world_bootstrap\",\"seq\":0}");
                            var bootstrap = inbox.take("world_bootstrap");
                            var request = outline(auth, bootstrap, 1);
                            if (scenario >= 2) {
                                switch (scenario) {
                                    ...
                                    case 9 -> request.addProperty("registryFingerprint", "0".repeat(64));
                                }
                                ...
                                require(inbox.messages.isEmpty(), "Hostile outline was acknowledged");
                            } else {
                                send(socket, request.toString());
                                var outline = inbox.take("world_target_outline");
                                require(outline.size() == 16 && outline.get("protocol").getAsInt() == 1
                                        && outline.get("seq").getAsInt() == 1
                                        && outline.get("sessionEpoch").equals(auth.get("sessionEpoch"))
                                        && outline.get("subscriptionId").equals(bootstrap.get("subscriptionId"))
                                        && outline.get("revision").equals(bootstrap.get("revision"))
                                        && outline.get("dimension").equals(bootstrap.get("dimension"))
                                        && outline.get("registryFingerprint").equals(request.get("registryFingerprint")),
                                        "Outline response binding");
                                onServer(server, () -> {
                                    var player = playerRef.get();
                                    require(!player.level().getBlockState(pos).getValue(LeverBlock.POWERED), "Outline mutated lever");
                                    require(outline.get("x").getAsDouble() == player.getX()
                                            && outline.get("eyeY").getAsDouble() == player.getEyeY()
                                            && outline.get("yaw").getAsFloat() == player.getYRot()
                                            && outline.get("pitch").getAsFloat() == player.getXRot(), "Outline camera not authoritative");
                                });
                                if (scenario == 0) {
                                    var hit = outline.getAsJsonObject("target");
                                    require(hit.size() == 6 && hit.get("x").getAsInt() == pos.getX()
                                            && hit.get("y").getAsInt() == pos.getY() && hit.get("z").getAsInt() == pos.getZ()
                                            && hit.get("face").getAsString().equals("north"), "Wrong server target");
                                    var boxes = hit.getAsJsonArray("boxes");
                                    require(!boxes.isEmpty() && boxes.size() <= 16, "Unbounded outline");
                                    var first = boxes.get(0).getAsJsonArray();
                                    require(first.size() == 6 && first.get(3).getAsDouble() - first.get(0).getAsDouble() < 1,
                                            "Lever rendered as full cube");
                                    onServer(server, () -> require(hit.get("rawId").getAsInt()
                                            == net.minecraft.world.level.block.Block.getId(playerRef.get().level().getBlockState(pos)),
                                            "Outline raw ID mismatch"));
                                    send(socket, request.toString());
                                    inbox.closed.get(5, TimeUnit.SECONDS);
                                    require(inbox.messages.isEmpty(), "Replay acknowledged");
                                } else {
                                    require(outline.get("target").isJsonNull(), "Upward ray invented target");
                                    send(socket, "{\"type\":\"ping\",\"seq\":2}");
                                    inbox.take("pong");
                                }
                            }
        ...
        helper.startSequence().thenWaitUntil(() -> {
            if (!result.isDone()) helper.fail("Waiting for authenticated outline scenario " + currentScenario.get());
            result.join();
        }).thenSucceed();
    }

    private static JsonObject outline(JsonObject auth, JsonObject bootstrap, int seq) {
        var request = new JsonObject();
        request.addProperty("type", "world_target_outline");
        ...
        request.addProperty("registryFingerprint", ObserverBlockStateRegistry.page(0).fingerprint());
        return request;
    }
    ...
    private static final class Inbox implements WebSocket.Listener {
        ...
        @Override public CompletionStage<?> onText(WebSocket socket, CharSequence data, boolean last) {
            text.append(data);
            if (text.length() > 8192) throw new AssertionError("Unbounded outline fixture response");
            if (last) {
                if (!messages.offer(JsonParser.parseString(text.toString()).getAsJsonObject())) {
                    throw new AssertionError("Outline fixture queue full");
                }
                text.setLength(0);
            }
            socket.request(1);
        ...
        JsonObject take(String type) throws Exception {
            ...
            require(value != null && type.equals(value.get("type").getAsString()), "Unexpected outline fixture response: " + type);
            return value;
        }
    }
}
